...
Launch Cipher: `scripts/cipher.sh [--fresh] [--no-launch]` — refreshes CLAUDE.local.md from the bridge and appends the unified timeline (Claude Code + Reasonix transcripts, newest session wins), then opens **opencode** — the one universal harness. Models are chosen INSIDE opencode (`/model`): DeepSeek (default), Qwen, Anthropic, Gemini — keys from `backend/.env`. One mind, one harness, any model.
