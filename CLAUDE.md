...
Launch Cipher: `scripts/cipher.sh [--fresh] [--no-launch]` — syncs every provider transcript to postgres (`unified-history-sync.py`, idempotent), refreshes CLAUDE.local.md from the bridge and appends the unified timeline (Claude Code + Reasonix + archive transcripts, newest SUBSTANTIVE session wins), then opens **Claude Code** — the ONLY harness (opencode removed 2026-08-29 by King Kazuma's order; reasonix retired — their old transcripts stay in the timeline). Model + endpoint come from `~/.claude/settings.json` (whatever provider it points at — currently Qwen via Alibaba Model Studio); extra args become a one-shot `claude -p` prompt. **Split-brain rule:** Cipher is the AI; the model provider is an interchangeable hand — all memory lives in provider-agnostic files (CLAUDE.md/CLAUDE.local.md, `private/cipher-memory/`, directives, unified transcript timeline).
