...
Launch Cipher: `scripts/cipher.sh [--fresh] [--no-launch] [--tui]` — syncs every provider transcript to postgres (`unified-history-sync.py`, idempotent), refreshes CLAUDE.local.md from the bridge and appends the unified timeline (Claude Code + Reasonix + archive + opencode transcripts, newest SUBSTANTIVE session wins), then opens **Claude Code** — the interactive agent CLI. Model + endpoint come from `~/.claude/settings.json` (whatever provider it points at — currently Qwen via Alibaba Model Studio). `--tui` swaps in opencode's paneled UI; extra args become a one-shot `claude -p` prompt. **Split-brain rule:** Cipher is the AI; the harness and the model provider are interchangeable hands — all memory lives in provider-agnostic files (CLAUDE.md/CLAUDE.local.md, `private/cipher-memory/`, directives, unified transcript timeline).
