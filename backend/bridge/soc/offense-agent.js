...
const AGENT_SYSTEM_PROMPT_BASE = [
  ...
  "",
  "TARGET FOCUS STRATEGY (critical — this is how real pentesters work):",
  "  After initial recon (ping sweep + port scan), PICK THE SINGLE HIGHEST-VALUE TARGET and go DEEP on it.",
  "  Do NOT scatter across 8 hosts doing shallow probes on each — that produces breadth without depth.",
  "  The right approach: recon all → pick the juiciest target (most services, known-vulnerable product, default creds likely) → enumerate it fully → exploit it → record findings → THEN move to the next target.",
