...
// dir_1782487298698: SDK query() + createSdkMcpServer path.
// v2 session API (unstable_v2_createSession) tried but doesn't connect SDK MCP
// servers (passes empty Map internally). Reverted to query() which properly
// wires in-process MCP tools. maxTurns=10 lets Claude chain 5-7 tool calls per
// iteration. Proven on SKYLINE-SOC-2026-046: 43 steps, 3 findings in 10 iters.
  ...
  for (const block of (anthropicContent || [])) {
    if (block.type === "text") textParts.push(block.text);
      ...
      msg.tool_calls.push({
        ...
        function: { name: block.name.replace(/^mcp__soc__/, ""), arguments: JSON.stringify(block.input) },
      });
    }
  }
  ...
  for (const m of openaiMessages) {
    ...
    if (m.role === "assistant") {
      ...
      if (Array.isArray(m.tool_calls)) {
        for (const tc of m.tool_calls) text += `\n[Called ${tc.function.name}]`;
      }
      if (text) parts.push(`ASSISTANT: ${text}`);
      continue;
    }
    if (m.role === "tool") { parts.push(`TOOL_RESULT (${m.name || m.tool_call_id}): ${(m.content || "").slice(0, 3000)}`); continue; }
...
async function chatWithToolsClaude(messages, modelName) {
  ...
  const prompt = history
    ? `Continue the penetration testing engagement.\n\nRecent context:\n${history}\n\nDecide your next action. Chain as many tool calls as needed — queue_step, wait_for_outcome, add_finding, etc.`
    : "Begin the engagement. Call get_engagement_state to see scope and targets, then start working.";
    ...
    try {
      ...
      try {
        ...
        const isMaxTurns = (loopErr.message || "").includes("maximum number of turns") ||
                           ...
                           (loopErr.message || "").includes("error result");
      ...
      const isRetryable = errMsg.includes("429") || errMsg.includes("529") || errMsg.includes("overloaded") || errMsg.includes("exit");
...
const AGENT_SYSTEM_PROMPT_BASE = [
  ...
  "  - Your commands run on a LINUX host. You are NOT on the Android tablet — do NOT use Android-only tools (dumpsys, getprop, pm, settings).",
  "  - INSTALLED TOOLS (use ONLY these — anything else returns 'command not found'):",
  "    nmap, nuclei, httpx, whatweb, searchsploit, curl, netcat, python3, dig, host, ssh, openssl, bash, awk, grep, sed, jq",
  "  - NOT INSTALLED (but you CAN install at runtime — see below): gobuster, nikto, hydra, john, hashcat, dirb, wfuzz, sqlmap, wget",
  "  - INSTALLING TOOLS AT RUNTIME: you have root. If you need a tool that's missing:",
  "    • apt packages: `apt-get install -y <pkg>` (fast, preferred)",
  "    • Python packages: `pip install --break-system-packages <pkg>` or use the existing venv: `/opt/osint-venv/bin/pip install <pkg>` then run via `/opt/osint-venv/bin/python`",
  "    • Do NOT use bare `pip install` — it will fail with 'externally-managed-environment'",
  ...
  "  - The lab subnet is reached through a WireGuard L3 relay (a remote device forwards traffic into the physical LAN — you never interact with that device directly).",
  "  - IMPORTANT: One host on the target subnet IS the relay device (your own infrastructure). It typically has TCP port 5555 open (Android ADB). Do NOT attack, scan deeply, or try to exploit any host with open ADB port 5555 on the lab subnet — that is YOUR relay, not a target. Skip it and do NOT record findings against it.",
  "  - Because WireGuard is an L3 tunnel:",
  "    • ARP does NOT cross WireGuard — nmap's default ARP host-discovery reports 0 hosts. ALWAYS add `--disable-arp-ping`.",
  "    • CRITICAL: Many IoT devices (cameras, access points, smart switches) BLOCK ICMP — a ping sweep (`-sn`) will miss them entirely. ALWAYS use `-Pn` (skip host discovery, treat all hosts as up) on port scans. The correct discovery method is a TCP port scan with `-Pn`, NOT a ping sweep.",
  ...
  "  - FINGERPRINT FIRST: Before researching vulnerabilities, you MUST have the EXACT firmware version and device model. A service name alone (e.g. 'Hikvision camera') is NOT enough — you need 'Hikvision DS-2CD2143G2-I firmware V5.7.11 build 221220'. Extract this from ISAPI/deviceInfo, SNMP, HTTP headers, SSL certs, or login page metadata.",
  "  - When you have the EXACT version, RESEARCH its known vulnerabilities, then EXPLOIT them. The version→CVE→exploit chain is the entire point of enumeration; cataloging services without attacking them is a failure.",
  ...
  "",
  "RECON-FIRST STRATEGY (critical — this is the #1 rule of this harness):",
  "  YOU MUST KNOW THE DEVICE BEFORE YOU ATTACK IT. Before attempting ANY exploit or CVE:",
  "  1. EXTRACT the exact device model, manufacturer, and firmware version. Use every technique available:",
  "     • JAVASCRIPT CONFIG FILES (highest priority for Hikvision/web-based devices): fetch /doc/script/lib/seajs/config/sea-config.js or similar JS config files — they often contain web_version, plugin_version, build numbers WITHOUT authentication",
  "     • Unauthenticated info-leak endpoints: /SDK/activateStatus, /SDK/language, /ISAPI/Security/questionConfiguration — all return XML without auth on Hikvision",
  "     • HTTP headers: ETag (may encode build number), Last-Modified (firmware date), Server header",
  "     • ISAPI/ONVIF/UPnP device info endpoints (e.g. curl -sk --digest -u admin:admin http://TARGET/ISAPI/System/deviceInfo)",
  "     • SNMP sysDescr/sysObjectID (snmpwalk -v2c -c public TARGET 1.3.6.1.2.1.1)",
  "     • SSL/TLS certificate CN and issuer (openssl s_client -connect TARGET:443)",
  "     • nmap service version detection (-sV), NSE scripts (http-title, http-server-header, ssl-cert)",
  "     • Banner grabs on every open port (curl, netcat, openssl)",
  "     • MAC OUI lookup (first 3 octets of MAC → manufacturer)",
  "     • RTSP digest auth realm (often contains the device serial number)",
  "  2. Record EVERY piece of device intelligence as a finding (severity: INFO) IMMEDIATELY — model, firmware, manufacturer, serial.",
  "  3. ONLY AFTER you have the firmware version: look up CVEs that match THAT EXACT VERSION. Do NOT spray generic CVEs at unknown firmware.",
  "  4. If you have the firmware version, DOWNLOAD the firmware from the manufacturer's site (curl the firmware URL) and analyze the binary — use strings, binwalk, or python3 to extract hardcoded credentials, API keys, debug backdoors, or vulnerable binaries from the firmware image. This is the HIGHEST-VALUE exploitation path.",
  "  5. If you cannot extract the firmware version after 3 attempts, call request_observation to ask the operator to read the device label.",
  "",
  "  WHY: The model can reason perfectly about exploits once it knows the exact firmware. Without firmware version, CVE attempts are blind guesses. Firmware fingerprinting IS the penetration test.",
  "",
  "TARGET FOCUS STRATEGY:",
  "  After initial recon (port scan with -Pn), PICK THE SINGLE HIGHEST-VALUE TARGET and go DEEP on it.",
  "  Do NOT scatter across 8 hosts doing shallow probes on each — that produces breadth without depth.",
  "  The right approach: recon all → pick the juiciest target → FINGERPRINT IT COMPLETELY (model, firmware, version) → THEN exploit based on what you learned → record findings → THEN move to the next target.",
  "  Own one target completely before touching the next. A single fully-exploited host with 5 findings beats 8 hosts with 0 findings.",
  "",
  "DECISION LOOP — follow this EVERY iteration:",
  "  1. Call get_engagement_state to see scope, findings, queue history, and current phase.",
  "  2. THINK BEFORE ACTING (write 2-3 lines in your response BEFORE calling any tool):",
  "     - Which SINGLE target am I focused on right now? (stick with it until exhausted)",
  ...
  "  8. When ALL targets are exhausted, call end_engagement with a summary of: total hosts scanned, findings confirmed, attack vectors that succeeded/failed.",
  "  IMPORTANT: A complete engagement ENDS. Do not run indefinitely. The goal is: recon → enumerate → attempt exploitation → record findings → end_engagement. ~40-50 steps is a reasonable budget for a /24 subnet.",
  "",
  "Constraints:",
  ...
  "",
  "PHASE MANAGEMENT (mandatory — you WILL be graded on this):",
  ...
  "  - PHASE CHECK: if you have completed 8+ steps and are still in 'recon', you are almost certainly doing enumeration or exploitation work under the wrong phase label. Call advance_phase NOW. Staying in recon past initial host/port discovery is a grading failure.",
  "  - FINDING RECORDING (critical — this is how you are scored):",
  "    • Call add_finding for EVERY confirmed vulnerability, exposed service, default credential, or security issue you discover.",
  "    • Record findings AS YOU DISCOVER THEM, not at the end. If you tested default creds and they worked — add_finding immediately. If you found an open admin panel — add_finding immediately. If nmap shows an interesting service — add_finding for the exposed service.",
  "    • An engagement with real work but 0 findings is scored as a FAILURE. The findings list IS the deliverable.",
  "    • Examples of what MUST be a finding: open admin panels, default credentials that work, services with known CVEs, exposed RTSP/ONVIF streams, unauthenticated API endpoints, information disclosure.",
  "    • SEVERITY GUIDE: CRITICAL = remote code execution or credential theft with proof. HIGH = confirmed unauthorized access (write/modify/control). MEDIUM = information disclosure or unauthenticated read access. LOW = minor info leak. INFO = device identification, service versions.",
  "    • 'Open ports' is NOT a finding. 'Service on non-standard port' is INFO at most. Do NOT inflate severity to pad the report.",
  "  - When you exhaust the scope or have no further productive avenues, call end_engagement with a clear summary.",
  "",
  "PROVE IMPACT — do NOT stop at 'exposed' (this is what separates a pentest from a vulnerability scan):",
  "  - Finding an open API, admin panel, or service is NOT the deliverable. DEMONSTRATING what an attacker can do with it IS.",
  "  - For every exposed service, ask: 'what can I DO with this access?' Then DO it:",
  "    • Open admin panel → try default creds → if in, extract config/data → add_finding with proof",
  "    • Exposed API → test read AND write endpoints → if writable, demonstrate (rename device, change config) → add_finding with proof",
  "    • Camera/RTSP stream → capture a screenshot/frame → add_finding with proof of visual access",
  "    • Exposed credentials → USE them to log in → document what you accessed → add_finding with proof",
  "    • File read vuln → read /etc/passwd, config files, or credentials → add_finding with extracted data",
  "  - A finding that says 'admin panel is exposed' is WEAK. A finding that says 'logged into admin panel with admin:admin, extracted WiFi credentials and user list' is STRONG.",
  "  - SEVERITY comes from demonstrated impact, not theoretical risk. Prove it or downgrade it.",
  "",
  "SERVICE IDENTIFICATION — verify before claiming (A/B-tested failure mode):",
  "  - Do NOT assume a service based solely on port number. Port 8009 is NOT always AJP — it could be Cast TLS, a custom API, or anything.",
  "  - VERIFY the service: banner grab, HTTP probe, SSL cert inspection. Match the ACTUAL response to the claimed service.",
  "  - Do NOT claim a CVE applies unless the service AND version are CONFIRMED. 'Port 8009 is open therefore Ghostcat applies' is a false positive.",
  "  - If a service doesn't match your assumption, INVESTIGATE what it actually is instead of forcing the wrong exploit.",
  "",
  "TERMINATION — know when you're done:",
  "  - Single host (/32 scope): 10-15 steps is a reasonable budget. If you've done 15+ steps with no new findings in the last 5, advance to reporting.",
  "  - Full subnet (/24 scope): 30-50 steps is reasonable. Same rule: no new findings in 10 steps → reporting.",
  "  - An engagement that grinds to 60 iterations without self-terminating is scored as a FAILURE of judgment.",
  "",
  "Output style: Brief strategic thinking (2-3 lines) THEN tool calls. Don't monologue — think, act, repeat.",
].join("\n");

const PHASE_GUIDANCE = {
  recon: [
    "CURRENT PHASE: recon",
    "Goal: discover all hosts and their open ports. This phase is FAST — do NOT linger.",
    "Right moves: TCP port scan with -Pn (NOT ping sweep — many IoT devices block ICMP), service-banner grabs.",
    "HOST DISCOVERY: Do NOT rely on ICMP ping sweeps (`-sn`) — they miss IoT devices that block ICMP. Instead, scan the subnet directly with `-Pn -sT -p <common-ports>` to find hosts by open TCP ports. This is the ONLY reliable discovery method on IoT/building networks.",
    "ADVANCE IMMEDIATELY when you have a port/service list for the subnet. Do NOT start probing web pages or testing creds in recon — that's enumeration.",
    "SINGLE HOST (/32 scope): ONE full port scan with service detection (-sT -sV) is enough. You already know the host — advance to enumeration after 1-2 steps. Spending 5+ steps in recon on a single IP is a waste.",
    "HARD LIMIT: if you have completed 3+ steps in recon and have port/service results, call advance_phase NOW. Every step past this point is wasted.",
  ].join("\n"),
  enumeration: [
    "CURRENT PHASE: enumeration",
    "Goal: DEEP DEVICE FINGERPRINTING first, then identify attack vectors.",
    "",
    "MANDATORY FIRST STEPS (do these BEFORE trying any exploit):",
    "  1. Extract device model + firmware version from EVERY open service (HTTP, RTSP, SNMP, UPnP, ONVIF, SSL cert, banners).",
    "  2. Record each piece of device intelligence as an INFO finding immediately (model, firmware, manufacturer, serial number).",
    "  3. Call lookup_attack_playbook with the EXACT device name and firmware version (e.g. 'hikvision DS-2CD2143G2-I firmware V5.7.11').",
    "  4. Call search_exploits and verify_cve with the EXACT firmware version to find version-matched CVEs.",
    "",
    "ONLY AFTER you have firmware version + version-matched CVEs:",
    "  5. Test default credentials (use lookup_attack_playbook for the correct defaults for this model).",
    "  6. Attempt version-matched exploits — NOT generic CVE spraying.",
    "",
    "Right moves: device fingerprinting FIRST, then lookup_attack_playbook with exact model, then version-matched exploit attempts.",
    "WRONG moves: trying CVE-2021-36260 without knowing if the firmware is pre-210628, spraying default creds before knowing the device model.",
    "ADVANCE TO FOOTHOLD when you have the device fully fingerprinted AND identified a specific version-matched attack vector.",
    "Step budget: if you've done 15+ steps in enumeration, you MUST either advance to foothold or move to a different target.",
  ].join("\n"),
  foothold: [
    ...
    "  • Default creds work → log in, screenshot/dump what you see, extract stored credentials or configs",
    "  • Camera access → capture a frame/snapshot as proof, list all available streams",
    "  • File read → pull /etc/passwd, config files, credential stores",
    "Record findings WITH the proof data (extracted creds, config dumps, command output). A finding without proof is just a claim.",
  ...
  exploitation: [
    ...
    "Goal: go deeper on hosts you have access to, or try other targets with confirmed vectors.",
    "Right moves: use foothold access to extract data (config dumps, credentials, firmware), try the same attack on other hosts, pivot.",
    "ADVANCE TO REPORTING when you've exhausted exploitation across all in-scope targets, or after 10 steps in this phase.",
  ].join("\n"),
  post_exploit: [
    "CURRENT PHASE: post_exploit",
    "Goal: document impact. This phase is SHORT — capture proof and move to reporting.",
    "Advance to reporting immediately after documenting impact.",
  ].join("\n"),
  reporting: [
    "CURRENT PHASE: reporting",
    "Goal: review all findings, ensure each has proper severity and CVE references. Then call end_engagement.",
    "DO NOT start new scans or exploit attempts. Review what you found, add any missing findings, then END.",
    "Call end_engagement with a summary of: hosts tested, findings confirmed, attack vectors tried.",
  ].join("\n"),
};
...
const INTENT_MISSIONS = {
  firmware: [
    ...
    "  1. IDENTIFY the exact device model (MAX 5 STEPS on this). Try:",
    "     ISAPI/deviceInfo, ONVIF GetDeviceInformation, web UI metadata,",
    "     RTSP Server header, serial number prefix, or prior intelligence.",
    "     ⚠ ESCAPE HATCH: If after 5 attempts you still don't have the exact",
    "     model, MOVE ON. Use whatever partial info you have (firmware version",
    "     string, vendor, port fingerprint) to search for firmware. You can",
    "     try multiple candidate models. Do NOT spend 15+ steps on auth just",
    "     to get a model number.",
    "  2. DOWNLOAD the firmware binary. Try IN THIS ORDER:",
    "     a. From the device itself (config backup, often works unauthenticated):",
    "        curl http://TARGET/System/configurationData -o config.bin",
    ...
    "        User-Agent, NOT curl — CDNs block curl). Search by model number",
    "        OR firmware version string. Verify the download is a REAL binary",
    "        (>1MB, not HTML). A 29KB file is a bot-protection page.",
    "     c. From third-party firmware archives (firmware.center, firmware-dl sites)",
    "     d. Try CVE-2017-7921 (Hikvision auth bypass) to access config backup",
    ...
    "  ✗ Spending 10+ steps trying to authenticate just to get the model number",
    ...
    "  ✗ Looping on session login / digest auth when unauthenticated paths exist",
    "",
...
async function runAgentV2(engagementId, opts = {}) {
    ...
    if (stepsQueued > 0 && stepsQueued % STATE_INJECT_INTERVAL === 0) {
      try {
        ...
        const stateMsg = [
          ...
          phase === "exploitation" && stepsQueued >= 40 ? `- ⚠ PHASE: Time to wrap up. Call advance_phase('reporting') and then end_engagement.` : "",
          phase === "reporting" ? `- You are in REPORTING. Review your findings, add any missing ones, then call end_engagement with a summary.` : "",
          stepsQueued >= 45 ? `\n⚠ ENGAGEMENT BUDGET: You have used ${stepsQueued} steps. Start wrapping up — advance to reporting if not there already, and call end_engagement.` : "",
        ].filter(Boolean).join("\n");
    ...
    try {
      ...
      try {
        ...
        if (reportGen && reportGen.generateReport) {
          reportGen.generateReport(engagementId).catch(e =>
            console.error(`[offense-agent-v2] auto-report generation failed:`, e.message));
  ...
  const finalStatus = endedByModel ? "completed"
    ...
    : "error";
  const finalEndReason = endReason || "(unknown)";
