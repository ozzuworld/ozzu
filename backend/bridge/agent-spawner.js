...
function buildIosDeployCommand(directive) {
  return [
    ...
    `gh run watch "$RUN_ID" --exit-status`,
    `echo "Caching IPA from the ios-latest Release locally..."`,
    `rm -rf /tmp/ozzu-ipa-cache && mkdir -p /tmp/ozzu-ipa-cache && gh release download ios-latest --dir /tmp/ozzu-ipa-cache -R ozzuworld/ozzu --clobber`,
