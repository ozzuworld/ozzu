...
const STAGES = [
  { key: "preparing", label: "PREPARING", color: "#737373" },
  { key: "customs_clearance", label: "CUSTOMS", color: "#F59E0B" },
  { key: "in_transit", label: "IN TRANSIT", color: "#3B82F6" },
  { key: "arrived", label: "ARRIVED", color: "#8B5CF6" },
  { key: "delivered", label: "DELIVERED", color: "#22C55E" },
  { key: "paid", label: "PAID", color: "#06B6D4" },
] as const;
      ...
      >
        ...
        {grouped.map((stage) => (
          <View key={stage.key} style={{ marginBottom: 16 }}>
            <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 8 }}>
              ...
              <Text style={{ color: "#525252", fontFamily: "monospace", fontSize: 10 }}>({stage.items.length})</Text>
            </View>
            {stage.items.length === 0 ? (
              <View style={{ backgroundColor: "#1A1A1A", borderRadius: 8, padding: 12, borderWidth: 1, borderColor: "rgba(255,255,255,0.04)" }}>
                <Text style={{ color: "#404040", fontFamily: "monospace", fontSize: 11, textAlign: "center" }}>No shipments</Text>
              </View>
            ) : (
              stage.items.map((sh) => (
        ...
        {shipments.length === 0 && !loading && (
          <View style={{ alignItems: "center", paddingVertical: 40 }}>
            ...
            <Text style={{ color: "#737373", fontFamily: "monospace", fontSize: 13 }}>No shipments yet</Text>
            <Text style={{ color: "#525252", fontFamily: "monospace", fontSize: 11, marginTop: 4 }}>Add your first coffee shipment</Text>
          </View>
        )}
      </ScrollView>
  ...
  return (
    ...
    >
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 6 }}>
        <Text style={{ color: "#E5E5E5", fontFamily: "monospace", fontSize: 13, fontWeight: "bold" }}>
      ...
      <View style={{ flexDirection: "row", gap: 12 }}>
        {shipment.coffee_type && (
          <Text style={{ color: "#A3A3A3", fontFamily: "monospace", fontSize: 10 }}>{shipment.coffee_type}</Text>
        )}
        {shipment.quantity_kg != null && (
          <Text style={{ color: "#737373", fontFamily: "monospace", fontSize: 10 }}>{shipment.quantity_kg}kg</Text>
        )}
        {shipment.bags_count != null && (
          <Text style={{ color: "#737373", fontFamily: "monospace", fontSize: 10 }}>{shipment.bags_count} bags</Text>
        )}
      </View>
      {shipment.buyer_name && (
        <Text style={{ color: "#525252", fontFamily: "monospace", fontSize: 10, marginTop: 4 }}>Buyer: {shipment.buyer_name}</Text>
      )}
      {(shipment.ship_date || shipment.estimated_arrival) && (
        <Text style={{ color: "#404040", fontFamily: "monospace", fontSize: 9, marginTop: 4 }}>
