          ...
          <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
            {editing ? (
              <>
                ...
                <Text style={{ color: colors.gray[400], fontFamily: "monospace", fontSize: 10, marginBottom: 6 }}>TYPE</Text>
                <View style={{ flexDirection: "row", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
                  {CONTACT_TYPES.map(t => (
                    <Pressable key={t} onPress={() => setType(t)} style={{
                      paddingHorizontal: 10, paddingVertical: 5, borderRadius: 6,
                      backgroundColor: type === t ? ACCENT + "22" : colors.gray[800],
                      borderWidth: 1, borderColor: type === t ? ACCENT + "44" : "rgba(255,255,255,0.06)",
                    }}>
                      <Text style={{ color: type === t ? ACCENT : colors.gray[300], fontFamily: "monospace", fontSize: 10, textTransform: "uppercase" }}>{t}</Text>
                    </Pressable>
                  ))}
                </View>
                ...
                <View style={{ flexDirection: "row", gap: 10, marginTop: 8 }}>
                  <Pressable onPress={save} disabled={saving} style={{ flex: 1, backgroundColor: ACCENT, borderRadius: 10, padding: 14, alignItems: "center" }}>
                    <Text style={{ color: colors.gray[850], fontFamily: "monospace", fontSize: 13, fontWeight: "bold" }}>{saving ? "Saving..." : "SAVE"}</Text>
                  </Pressable>
                  <Pressable onPress={() => setEditing(false)} style={{ flex: 1, backgroundColor: colors.gray[800], borderRadius: 10, padding: 14, alignItems: "center", borderWidth: 1, borderColor: "rgba(255,255,255,0.06)" }}>
                    <Text style={{ color: colors.gray[300], fontFamily: "monospace", fontSize: 13 }}>CANCEL</Text>
                  </Pressable>
                </View>
              </>
            ) : contact ? (
              <>
                ...
                {contact.notes && (
                  <View style={{ backgroundColor: colors.gray[800], borderRadius: 10, padding: 14, marginBottom: 12 }}>
                    <Text style={{ color: colors.gray[400], fontFamily: "monospace", fontSize: 9, letterSpacing: 2, marginBottom: 6 }}>NOTES</Text>
                    <Text style={{ color: colors.gray[200], fontFamily: "monospace", fontSize: 12 }}>{contact.notes}</Text>
                  </View>
                )}
                <Pressable onPress={handleDelete} style={{ backgroundColor: "#EF444422", borderRadius: 10, padding: 12, alignItems: "center", marginTop: 8 }}>
                  <Text style={{ color: colors.error, fontFamily: "monospace", fontSize: 12, fontWeight: "bold" }}>DELETE CONTACT</Text>
                </Pressable>
              </>
            ) : (
...
function DetailRow({ label, value }: { label: string; value: string | null | undefined }) {
  ...
  return (
    <View style={{ flexDirection: "row", justifyContent: "space-between", paddingVertical: 4 }}>
      ...
      <Text style={{ color: colors.gray[50], fontFamily: "monospace", fontSize: 11, textTransform: label === "Type" ? "uppercase" : "none" }}>{value}</Text>
    </View>
  );
}
