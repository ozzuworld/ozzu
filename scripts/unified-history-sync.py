...
def main():
    ...
    for cand in candidates:  # already newest-first
        ...
        try:
            ...
            if resp.get('duplicate'):
                pass  # bridge already had it — fine
            ingested_now += 1
    ...
    if args.quiet:
        print(f"history-sync: {ingested_now} ingested, "
              f"{skipped_noise} noise skipped, {len(ingested)} already known")
    else:
        print(f"Ingested {ingested_now} session(s); {skipped_noise} trivial "
              f"session(s) skipped; {len(ingested)} already in postgres; "
              f"{errors} error(s).")
    return 0
