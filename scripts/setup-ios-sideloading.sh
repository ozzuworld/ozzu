...
# Bumped 2026-05-16 (dir_1778958643514). 0.6.3 was released 2026-05-05.
SIDESTORE_VERSION="0.6.3"
...
ALTSERVER_URL="https://github.com/NyaMisty/AltServer-Linux/releases/download/${ALTSERVER_VERSION}/AltServer-x86_64"

echo "=== iOS Sideloading Setup ==="
echo ""

# ── Step 1: Check dev-01 connectivity ──
echo "[1/6] Checking dev-01 connectivity..."
...
echo ""
echo "[2/6] Downloading Sideloader CLI v${SIDELOADER_VERSION}..."
TMPDIR=$(mktemp -d)
...
echo ""
echo "[3/6] Downloading SideStore v${SIDESTORE_VERSION}..."
wget -q -O "$TMPDIR/SideStore.ipa" "$SIDESTORE_URL"
echo "  Downloaded."

# ── Step 4: Download AltServer-Linux ──
echo ""
echo "[4/6] Downloading AltServer-Linux ${ALTSERVER_VERSION}..."
wget -q -O "$TMPDIR/AltServer" "$ALTSERVER_URL"
chmod +x "$TMPDIR/AltServer"
echo "  Downloaded."

# ── Step 5: Transfer to dev-01 ──
echo ""
echo "[5/6] Transferring files to dev-01..."
ssh "$DEV01" "mkdir -p ~/bin ~/ozzu-ios-setup"
scp -q "$TMPDIR/sideloader-cli-x86_64-linux-gnu" "$DEV01:~/bin/sideloader"
scp -q "$TMPDIR/AltServer" "$DEV01:~/bin/AltServer"
scp -q "$TMPDIR/SideStore.ipa" "$DEV01:~/ozzu-ios-setup/SideStore.ipa"
ssh "$DEV01" "chmod +x ~/bin/sideloader ~/bin/AltServer"
echo "  Sideloader installed to ~/bin/sideloader on dev-01"
echo "  AltServer installed to ~/bin/AltServer on dev-01"
echo "  SideStore.ipa saved to ~/ozzu-ios-setup/ on dev-01"

# ── Step 6: Start Anisette on GCP VM ──
echo ""
echo "[6/6] Setting up Anisette v3 server on GCP VM..."
